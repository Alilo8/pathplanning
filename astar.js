...
class Astar{
    ...
    for(let i=0; i<6; i++){
        ...
        if(0 <= temp[0] && temp[0] < 50 && 0 <= temp[1] && temp[1] < 20 && 0 <= temp[2] && temp[2] < 50){
          if(!(this.checkOpenSet(temp)) && !(this.checkCloseSet(temp)) && !(this.checkCollision(temp))){
              ...
              this.open_set.set(temp,this.heuristic(temp, this.end));
  ...
  checkCloseSet(item){
      for (let i=0; i<this.close_set.length; i++){
        if (this.close_set[i][0] == item[0] && this.close_set[i][1] == item[1] && this.close_set[i][2] == item[2]){
          return true
        }
      }
      return false
  ...
  find_path(){
    ...
    while(true){
      ...
      if(this.arrayIsEqual(current, this.start))
       break
