...
export default class RBT{
    ...
    targetFollow(current){
        ...
        const new_node = [point.x + normal.x * 0.1, point.y + normal.y * 0.1, point.z + normal.z * 0.1];
        if(this.checkExistence(new_node, new_node)){
            return false;
        }
    ...
    wallFollow(parent){
        ...
        // if(value>0){
        //     current[index] = Math.ceil(current[index]) + 0.1;
        // }
        // else{
        //     current[index] = Math.ceil(current[index]-1) - 0.1;
        // }
        current[index] = Math.round(current[index])
        let current_obs;
        ...
        if(!dir_next)
            ...
            if(current[index] == parent[index]){
                ...
                if(this.checkCollision(current_obs))
                    current[index] += 1;
            }
            if(value>0)
                current[index] -= 0.1;
        ...
        while(true){
            ...
            if(check){
                if(this.checkExistence(current, parent)){
                    return;
                }
    ...
    checkExistence(temp, parent){
        for(const node of this.nodes){
            if(this.getDistance(node, temp) < 0.3)
    ...
    getCost(child, parent){
        return this.getDistance(child, this.end);
    }
    getDistance(from, to){
